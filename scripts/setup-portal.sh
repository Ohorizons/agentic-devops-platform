...
step1_portal() {
  ...
  ask PORTAL_DOMAIN "Portal domain (e.g. portal.acme.com)" ""
  if [[ -z "$PORTAL_DOMAIN" ]]; then
    echo -e "${RED}Portal domain is required${NC}"
    exit 1
  fi
...
step3_azure() {
  ...
  ask ACR_NAME "Azure Container Registry name (e.g. acrmyproject)" ""
  if [[ -n "$ACR_NAME" ]]; then
    ok "ACR: $ACR_NAME"
  else
    ACR_NAME="acr${PORTAL_NAME//[^a-zA-Z0-9]/}"
    info "ACR name auto-generated: $ACR_NAME"
  fi
...
generate_azure_config() {
  ...
  local helm_file="$ROOT_DIR/deploy/helm/backstage-values-${ENVIRONMENT}.yaml"
...
TFVARS
  ...
  # --- Generate Helm values for client ---
  cat > "$helm_file" << HELMVALUES
# =============================================================================
# Generated by setup-portal.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
# Backstage AKS Deployment — ${PORTAL_NAME}
# =============================================================================
...
ingress:
  ...
  tls:
    - hosts:
        - ${PORTAL_DOMAIN}
      secretName: tls-${PORTAL_NAME//[^a-zA-Z0-9-]/-}
...
backstage:
  ...
  image:
    ...
    repository: backstage/${PORTAL_NAME}
    tag: "v1.48.3"
  ...
  appConfig:
    app:
      title: ${PORTAL_NAME} | Agentic DevOps Platform
      baseUrl: https://${PORTAL_DOMAIN}
    organization:
      name: ${PORTAL_NAME} — Agentic DevOps Platform
    backend:
      baseUrl: https://${PORTAL_DOMAIN}
      listen:
        port: 7007
    ...
    catalog:
      ...
      locations:
        - type: url
          target: https://raw.githubusercontent.com/${GITHUB_ORG}/${TEMPLATE_REPO##*/}/main/backstage/catalog-info.yaml
          rules:
            - allow: [Component]
        - type: url
          target: https://raw.githubusercontent.com/${GITHUB_ORG}/${TEMPLATE_REPO##*/}/main/backstage/examples/org.yaml
          rules:
            - allow: [User, Group, Domain, System]
        - type: url
          target: https://raw.githubusercontent.com/${GITHUB_ORG}/${TEMPLATE_REPO##*/}/main/backstage/examples/entities.yaml
          rules:
...
HELMVALUES
  ...
  ok "Generated $helm_file"

  ok "Configuration ready"
  echo ""
  info "Generated files:"
  echo -e "  ${BOLD}$tfvars_file${NC}"
  echo -e "  ${BOLD}$helm_file${NC}"
  echo ""
  ...
  echo ""
  info "To deploy Backstage via Helm:"
  echo -e "  ${BOLD}helm upgrade --install backstage backstage/backstage \\${NC}"
  echo -e "  ${BOLD}  --namespace backstage --create-namespace \\${NC}"
  echo -e "  ${BOLD}  --values deploy/helm/backstage-values-${ENVIRONMENT}.yaml --wait${NC}"
}
...
summary() {
  ...
  echo -e "  ${BOLD}Portal Domain:${NC}   $PORTAL_DOMAIN"
